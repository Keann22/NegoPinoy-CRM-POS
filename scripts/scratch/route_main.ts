...
export async function GET() {
  try {
    ...
    // self-heal cleanups above must still run first ΓÇö they mutate the drafts
    // these queries read.)
    //
    // 3a. Bundle products consume a target component's physical stock too
    // (e.g. "Wok Pan with Takip" = 1x Wok Pan + 1x Cover) ΓÇö an order for the
    ...
    const [
      ...
      // it ΓÇö checked below once demand is computed. A deficit fully explained
    ...
    // A bundle carries no purchasable stock of its own ΓÇö suppliers only sell
    ...
    // them count as unfulfilled ΓÇö items without an issue were successfully
    ...
    // Shipping/For Pick-up), buying more wouldn't help that order ΓÇö it's
    // already handled ΓÇö so it doesn't belong on a "what do we need to buy"
    ...
    // them. These aren't a purchasing task, but the deficit is still real ΓÇö
    ...
    // recorded (see procurement_request_sources) ΓÇö lets "Staff Req." trace
...
export async function POST(req: Request) {
  try {
    ...
    for (const p of purchases) {
      ...
      // products.supplier_id null ΓÇö so every previously-bought item kept
