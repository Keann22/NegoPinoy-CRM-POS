...
async function main() {
  ...
  // Same cross-reference impact check as the first merge pass.
  const referenceCounts = {};
  for (const { table, col } of REPOINT_TABLES) {
    const { count, error } = await supabase.from(table).select('id', { count: 'exact', head: true }).in(col, retireIds);
    if (error) { console.log(`  (skipping ${table}: ${error.message})`); continue; }
    referenceCounts[table] = count || 0;
  }
  const { data: childRows } = await supabase.from('products').select('id, name, parent_id').in('parent_id', retireIds);
  const assemblyReferences = [];
  for (const p of all) {
    if (!p.assembly_recipe || !Array.isArray(p.assembly_recipe)) continue;
    for (const comp of p.assembly_recipe) {
      if (retireIds.includes(comp.productId)) assemblyReferences.push({ bundleId: p.id, bundleName: p.name, componentId: comp.productId });
    }
  }

  console.log('=== CROSS-REFERENCE IMPACT ===');
  for (const [table, count] of Object.entries(referenceCounts)) console.log(`  ${table}: ${count}`);
  console.log(`  products.parent_id: ${childRows?.length || 0}`);
  console.log(`  assembly_recipe (JSON): ${assemblyReferences.length}`);
  console.log('');
  ...
  for (const plan of plans) {
    ...
    if (plan.newStock !== (plan.primary.stock_level ?? 0)) {
      const { error: stockErr } = await supabase.from('products').update({ stock_level: plan.newStock }).eq('id', plan.primary.id);
    ...
    for (const retire of plan.retire) {
      ...
      const { error: parentErr } = await supabase.from('products').update({ parent_id: plan.primary.id }).eq('parent_id', retire.id);
  ...
  console.log('Merge complete. Verifying no duplicate groups remain (broadened definition)...\n');
  ...
  const remaining = Array.from(afterGroups.values()).filter(g => g.length > 1);
  console.log(remaining.length === 0 ? 'Confirmed: 0 duplicate groups remain.' : `WARNING: ${remaining.length} groups still remain.`);
