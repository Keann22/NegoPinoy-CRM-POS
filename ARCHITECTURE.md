...
| `notifications` | Picker app (issue reported), Packer app (order packed), `POST /api/staff-messages` (`source: 'staff_message'`, one row per tagged recipient) | Bell-icon notifications for the sales rep who owns the order; `StaffMessageFab`'s unread badge filters to `source = 'staff_message'` |
...
`notifications.source` (added by `scripts/migrations/add_source_to_notifications.sql`, run manually — no migration runner in this repo) is set to `'staff_message'` on these fan-out rows specifically so `StaffMessageFab`'s unread badge can count *only* staff-tagged messages, separate from system notifications (order packed, issue reported) that share the same table but leave `source` null.
