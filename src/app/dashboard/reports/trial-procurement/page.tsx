...
export default function TrialProcurementSheet() {
  ...
  const handleWipeGhostStock = async () => {
    if (!reconciliationItems || reconciliationItems.length === 0) {
      return alert("No ghost stock to wipe.");
    }
    if (!confirm(`Are you sure you want to reset stock to 0 for ${reconciliationItems.length} items?`)) return;

    try {
      const productIds = reconciliationItems.map((item: any) => item.productId);
      const res = await fetch('/api/inventory/procurement/wipe-ghost-stock', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ productIds })
      });
      if (!res.ok) throw new Error(await res.text());
      alert('Ghost stock wiped successfully!');
      fetchData(true);
    } catch (e: any) {
      alert("Failed to wipe stock: " + e.message);
    }
  };

  if (loading) return <div className="p-8 text-center text-slate-500">Loading Trial Procurement Sheet...</div>;
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-6 space-y-8 bg-white shadow rounded-lg md:mt-8">
      <div className="flex flex-col md:flex-row justify-between md:items-center gap-4 border-b pb-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-red-600">Trial Procurement Sheet</h1>
          <p className="text-slate-600 text-sm md:text-base">Testing ground for wiping ghost negative stock.</p>
        </div>
      ...
      {reconciliationItems.length > 0 && (
        <div className="border border-red-200 bg-red-50 rounded-lg p-6">
          <div className="flex justify-between items-start mb-4">
            <div>
              <h2 className="text-xl font-bold text-red-800">Ghost Negative Stock ({reconciliationItems.length})</h2>
              <p className="text-red-600 text-sm">These items have negative stock but NO active open orders. Wiping them will reset their stock to 0.</p>
            </div>
            <Button onClick={handleWipeGhostStock} variant="destructive" className="font-bold">
              Wipe Out Ghost Stock
            </Button>
          </div>
            ...
            <table className="w-full text-sm text-left">
              <thead className="bg-slate-100 border-b">
                <tr>
                  ...
                  <th className="p-2 font-semibold text-center">Current Stock</th>
                  <th className="p-2 font-semibold text-center">Explanation</th>
                </tr>
              </thead>
              <tbody>
                {reconciliationItems.map((item: any) => (
                  <tr key={item.productId} className="border-b">
                    ...
                    <td className="p-2 text-center text-red-600 font-bold">{item.currentStock}</td>
                    <td className="p-2 text-center text-slate-500">
                      {item.explainingOrder ? (
                        <>Was in order {item.explainingOrder.shortOrderId} ({item.explainingOrder.status})</>
                      ) : (
                        "No order found"
                      )}
                    </td>
