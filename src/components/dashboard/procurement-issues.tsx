...
export function ProcurementIssues({ isAdmin }: { isAdmin?: boolean }) {
  ...
  const fetchIssues = async () => {
    try {
      setIsLoading(true);
      const res = await fetch('/api/inventory/procurement-issues');
  ...
  const openIssue = async (issue: any) => {
    ...
    try {
      const res = await fetch(`/api/inventory/procurement-issues?id=${issue.id}`);
      if (res.ok) {
    ...
    try {
      ...
      const res = await fetch(`/api/inventory/procurement-issues/orders?productId=${issue.product_id}`);
      if (res.ok) {
        const data = await res.json();
  ...
  const handleSendReply = async () => {
    ...
    setIsSending(true);
    try {
      const res = await fetch('/api/inventory/procurement-issues/messages', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      const refetch = await fetch(`/api/inventory/procurement-issues?id=${selectedIssue.id}`);
      if (refetch.ok) {
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-sm text-slate-500 py-4 text-center bg-white rounded border border-amber-100">
              No active procurement issues at the moment.
            </div>
          ) : (
