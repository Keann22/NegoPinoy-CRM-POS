...
**On-Hold badge on the demand/issue surfaces (added 2026-08-21).** An order manually paused via `on-hold-reason-dialog.tsx` (which sets `orders.status = 'On-Hold'`) now carries an amber "⏸ On Hold" pill everywhere it surfaces in the dashboard's demand/issue views, so a paused order reads at a glance instead of blending in as a generic status. Three surfaces: (1) the **Order Issues** card (`order-issues.tsx`) — the list branch of `GET /api/inventory/issues` now selects `orders.status` (previously only `id, customer_id, customers`), and the grouped card renders the pill under the order number; (2) the **Overdue Orders** grid (`OverdueOrders.tsx`) — keyed off the already-loaded `order.orderStatus`, alongside the existing "Stock issue reported" / "Still out of stock" badges; (3) the **Total Open Demand / reserved-stock** pop-up (`reserved-stock-dialog.tsx`) — On-Hold rows render the amber pill instead of the plain grey status `Badge`. This is **display-only and changes no demand math**: On-Hold orders still count toward **Total Open Demand** (they hold allocated stock — stock is deducted at order creation) but are excluded from **Need to Buy** (`UNFULFILLED_STATUSES` in `procurement-service.ts` is `Processing` / `Picked (with issue)` / `Waiting for Stock`, and omits `On-Hold`), so a paused order never generates a purchase suggestion — the badge just explains why the reservation is still holding stock.
...
**An order's own sales rep is always an affected party, tagged or not (added 2026-08-04).** Messaging *about an order* — the Message Staff dialog (pick an order) and any reply in an order-linked issue thread — now always notifies `orders.sales_person_name`, on top of whoever the sender explicitly tagged. Before this, selecting an order only attached the message to it; the person who actually owns/is affected by the order got nothing unless someone remembered to tag them by hand. The affected-rep concept already existed for **product** shortages (`getAffectedSalesReps`/`getAffectedSalesRepOrders(productId)` in `procurement-service.ts`, used by Bulk Receive and the procurement-issue report) — this extends the same "ping who's blocked" idea to the order-message path, which is simpler because an order has exactly one rep:
- **Message Staff** (`createStaffMessage` in `src/lib/services/staff-message-service.ts`): when `issueType === 'order'`, it looks up the order's `sales_person_name` and merges it into the recipient list used for the first message's `mentions`, the thread-member registration, and the Bell fan-out. Deduped case-insensitively and never the sender.
- **Replies** (`POST /api/inventory/issues/messages`): resolves the thread's `order_id` up front and, if set, adds that order's `sales_person_name` to the participant pool alongside the whole-thread fan-out above. So a reply reaches the order's rep even on a thread they never posted in and weren't tagged on.

This was verified live against production with a controlled end-to-end call (fake sender + placeholder tag on an owner-held order): two `notifications` rows resulted — one for the tagged placeholder, one for the order's rep who was never tagged — then the test rows were deleted.
